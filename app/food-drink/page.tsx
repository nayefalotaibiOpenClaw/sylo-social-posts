  ...
  return (
    <div className="h-screen flex overflow-hidden bg-gray-50">
      <div className="w-[72px] bg-white border-r border-gray-200 flex flex-col items-center py-4 gap-1 shrink-0">
        ...
        <button onClick={() => setActiveTab("settings")} className={`w-14 h-14 rounded-xl flex flex-col items-center justify-center gap-0.5 transition-all ${activeTab === "settings" ? "bg-orange-600 text-white" : "text-gray-500 hover:bg-gray-100"}`}>
          <Settings size={20} />
          <span className="text-[10px] font-medium">Settings</span>
        </button>
        <button onClick={() => setActiveTab("theme")} className={`w-14 h-14 rounded-xl flex flex-col items-center justify-center gap-0.5 transition-all ${activeTab === "theme" ? "bg-orange-600 text-white" : "text-gray-500 hover:bg-gray-100"}`}>
          <Palette size={20} />
        ...
        <div className="w-[280px] bg-white border-r border-gray-200 p-5 overflow-y-auto">
          ...
          {activeTab === "settings" && (
            <div className="space-y-6">
              <button onClick={() => setEditMode(!editMode)} className={`w-full py-2.5 rounded-lg text-xs font-bold border ${editMode ? "bg-orange-100 text-orange-900 border-orange-200" : "bg-gray-50 text-gray-500 border-gray-200"}`}>
              ...
              <div>
                ...
                <div className="flex gap-1">
                  {(["1:1", "9:16", "3:4", "4:3", "16:9"] as const).map((ratio) => (
                    <button key={ratio} onClick={() => setAspectRatio(ratio)} className={`flex-1 py-2 rounded-lg text-xs font-bold ${aspectRatio === ratio ? "bg-orange-600 text-white" : "bg-gray-50 text-gray-400"}`}>
                      {ratio}
                    </button>
                  ))}
                </div>
          ...
          {activeTab === "theme" && (
            <div className="space-y-4">
              <p className="text-xs text-gray-500">Theme for Food & Drink posts.</p>
      ...
      <main className="flex-1 overflow-y-auto p-12">
        <div className="max-w-6xl mx-auto mb-12 flex justify-between items-end">
          <div>
            <h1 className="text-4xl font-black text-orange-950">Food & Drink</h1>
            <p className="text-orange-900/60 font-bold mt-2">Layout templates for restaurants, cafes, and beverage brands</p>
          </div>
          <div className="flex gap-3">
            <button onClick={() => setSelectedPosts(FOOD_DRINK_POSTS.map((p) => p.id))} className="px-4 py-2 text-orange-900 font-bold text-sm">Select All</button>
            <button onClick={handleDownloadSelected} disabled={downloading || selectedPosts.length === 0} className="px-6 py-3 bg-orange-600 text-white rounded-xl font-bold flex items-center gap-2 disabled:opacity-50 transition-all active:scale-95">
